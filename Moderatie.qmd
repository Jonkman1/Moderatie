---
title: "Enkelvoudige, meervoudige en modererdende regressie"
format: html
editor: visual
---
...
Onlangs heb ik een groep studenten van Forensische Orthopedagogiek van de Universeriteit van Amsterdam begeleid bij het schrijven van hun masterscriptie. Ze hadden met elkaar een datasample verzameld. Ze moesten op zoek naar de relatie tussen cannabisgebruik en online-blootstelling aan cannabis. Daarboven op moesten ze antwoord geven op de vraag of bepaalde variabelen op de relatie van invloed zijn. Daarvoor moesten ze een moderatieanalyse uitvoeren. 

In deze korte blog laat ik zien hoe ik deze techniek zou uitvoeren. Dat heb ik de studenten laten zien en hier deel ik het met anderen. Eerst introduceer ik heel kort moderatie-analyse als onderdeel van regressie-analyse en wat de achterliggende ideeën ervan. Dan lichtt ik kort de dataset uit die gebruikt is. Dan laat ik zien welke pakketten van R ik hierbij heb gebruikt en hoe deze pakketten ons werk hierbij kunnen ondersteunen. Dan laat ik twee soorten modernatieanalyses zien. Één analyse waarbij de moderator een categoriale variabele en een andere analyse waarbij de moderator een continue variabele is. Dan presenteer ik de tekst van de moderatieanalyse.
...
Voor deze analyse maak ik gebruik van enkkele pakketten die hierbij - Het pakket `haven` (https://haven.tidyverse.org/) heb ik gebruikt om het spss-databestand toegankelijk voor R te maken; - `Tidyverse` (https://www.tidyverse.org/) gebruik ik voor het bewerken van het bestand maar ook om de resultaten goed te kunnen visualiseren; - De pakketten `jtools` (https://jtools.jacob-long.com/) en `huxtable` (https://hughjonesd.github.io/huxtable/) zijn gebruikt om de modellen overzichtelijk in een tabel te kunnen afddrukken; - Het pakket `interactions` (https://bookdown.org/mike/data_analysis/interactions-package.html) is gebruikt om van moderatie inzichtelijke interactieplots te tonen. De pakketten moeten wel op jouw machine geïnstalleerd zijn om te kunnen gebruiken.
...
# MODERATIEANALYSE MET CONTINUE MODERATOR
...
# MODERATORANALYSE MET DICHOTOME MODERATOR
...
Mijn analyse en haar tekst (ingekort en in analyse aangepast) heb ik gebruikt om de resultaten ook op papier te zetten. Ik heb een opzet gemaakt voor `quarto` waar je goed wetenschappelijke artikelen kunt maken. De uiteindelijke tekst in pdf met resultaten vind je hier.
